...
def graph_tsne(
    ...
    tsne_coordinates = model.fit_transform(expression_data)

    # Set up the plot
    _, ax = plt.subplots()

    # Plot target cluster in grey
    target_clusters = tsne_coordinates[cluster_id == target_cluster_id]
    ax.scatter(
        target_clusters[:, 0], target_clusters[:, 1], color="grey", s=1, alpha=0.25
    )
    ...
    ax.set_title(f"t-SNE for gene {marker_gene} in cluster {target_cluster_id}")
...
def graph_gini(
    ...
    # Flatten the cluster_id array
    cluster_id = cluster_id.flatten()

    # Identify indices of cells in the target cluster
    cluster_index = np.where(cluster_id == target_cluster_id)[0]

    # Get spatial coordinates and assign labels to cells in target cluster
    cluster_coor = coordinate[cluster_index, :]
    cluster_lbl = np.ones(len(cluster_index))  # label these cells as '1'

    # Identify cells expressing marker gene in target cluster
    ...
    # Get spatial coordinates and assign labels to these cells
    ...
    if x.shape[1] != 2:
        raise ValueError("We need 2D data.")
    ...
    # Define colors
    colors = plt.get_cmap("Set1").colors  # better colors
    alphas = [1, 0.25]

    # For each cluster, compute Gini for each angle
